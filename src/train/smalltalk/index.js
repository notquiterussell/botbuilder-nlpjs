...
const parser = parse({ delimiter: '\t', from_line: 2 }, async (err, data) => {
  ...
  fs.writeFileSync(
    path.join(__dirname, '..', '..', '..', 'target', 'smalltalk-public.json'),
    JSON.stringify(publicCorpus)
  );
  fs.writeFileSync(
    path.join(__dirname, '..', '..', '..', 'target', 'smalltalk-private.json'),
    JSON.stringify(privateCorpus)
  );
});
